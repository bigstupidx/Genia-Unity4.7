...
public class VirtualJoystickRegion : MonoBehaviour {
	...
	public Vector2 origin;				// Touch's Origin in Screen-Space.
	...
	private void GetPosition() 
	{
		foreach (Touch touch in Input.touches) 
		{
			...
			if (fingerID >= 0 && fingerID < Input.touchCount) 
			{
				...
				if(Input.GetTouch(fingerID).position.x < StartPosition.x && Input.GetTouch(fingerID).position.y < StartPosition.y && Input.GetTouch(fingerID).phase == TouchPhase.Began) 
				{
